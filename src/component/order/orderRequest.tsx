...
import { RequestOrderDataType } from '@/types/order';
...
export default function OrderRequest() {
  ...
  const items: MenuProps['items'] = [
    {
      key: '1',
      label: (
        <a href={`/orders/order-request/${orderId}`} rel="noopener noreferrer" className='text-base'>
          View Order Request<br /> Details
        </a>
      ),
    },
    {
  ...
  const columns: ColumnsType<RequestOrderDataType> = [
    {
      title: 'Order ID',
      dataIndex: 'id',
  ...
  const data: RequestOrderDataType[] = [];
  for (let i = 0; i < 40; i++) {
    data.push({
      key: i,
      id: `Edward King ${i}`,
